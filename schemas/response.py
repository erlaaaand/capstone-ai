...
class MarketContextResponse(BaseModel):
    ...
    price_min_idr: int      = Field(..., description="Harga minimum per kg (IDR), dari data pasar terbaru.")
    price_max_idr: int      = Field(..., description="Harga maksimum per kg (IDR).")
    price_avg_idr: int      = Field(..., description="Harga rata-rata per kg (IDR).")
    sample_count:  int      = Field(..., description="Jumlah listing yang menjadi basis harga.")
...
class VarietyPriceSummary(BaseModel):
    ...
    price_min_idr: int      = Field(..., description="IDR per kg")
    price_max_idr: int      = Field(..., description="IDR per kg")
    price_avg_idr: int      = Field(..., description="IDR per kg")
    sample_count:  int      = Field(..., description="Jumlah listing sumber")
...
class MarketPricesResponse(BaseModel):
    ...
    variety_count: int                   = Field(default=0, description="Jumlah varietas dengan data harga.")
...
class MarketReportResponse(BaseModel):
    ...
    entries_discarded: int            = Field(default=0)
